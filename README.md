...
<li>Added experimental support for <code>repeating-linear-gradient</code>. The script produces output with correct angles (arguably the most important part); however, eagle-eyed observers will notice discrepancies from CSS rendering&#8212;this is especially the case when the first and last color stops are not equal to 0% and 100% respectively. Future updates may correct such discrepancies if possible.</li>
